...
- Scope: approved plan doc `docs/CHARACTER_PERSPECTIVE_TRIAL_EVIDENCE_PLAN_2026-07.md` (marked implemented); `scripts/compose-perspective-trial-evidence.mjs` — manifest-driven, deterministic evidence composer for Mage four-direction idle candidates with fail-closed machine gates (geometry, `binary_alpha`, `cell_bleed`, apparent-height parity ±1px, pivot contact row 47, occupancy bounds) measured on the read pixels. Per ChatGPT's independent review (2026-07-22), the manifest is now **bound to the canonical target and plates**: target geometry (32×48, front/back/left/right, pivot row 47) is resolved from `hero_actor_targets.json` and any substitute fails; the `farm_bright`/`woods_bridge` plates must be the exact committed `grass_a` master and `eldoria-placeholder.png` tileset (declared cells bounds-checked; bridge terrain labeled in every artifact); occupancy bounds are **mandatory** — omission fails, never skips. Per-candidate 1× previews on both plates, 8× NN sheet, pivot/baseline overlays, contact sheet, `eldoria-perspective-trial-report/v1` with sha256-locked inputs and named open judgment gates; cross-candidate comparison + trial index. `scripts/test-perspective-trial.mjs` — 19 cases (including substituted-target, substituted-plate, omitted-occupancy, and out-of-bounds-cell rejections), chained into `test:asset-pipeline` with no `.github/workflows/` change.
- Verification: `test:perspective-trial` 19/19 on the repaired head; full local suite (`check`, `test:visual-targets`, `test:asset-pipeline`, `test:unit`) green; `smoke`/`emulation` on required CI for the exact PR head.
