...
- Implementation notes:
  ...
  - Title-to-world and world-entry camera fades added; the title button waits for `FADE_OUT_COMPLETE` before `scene.start`.
  - The animated layers (tint tween, particle emission) and the fades are inert under `window.__ELDORIA_E2E__`: the controller still constructs everything (static first-stop tint, shadows) but skips per-frame work so the long smoke tests keep their original timing and determinism. Real dev/production play is unaffected. This resolved a CI smoke timeout where the always-on animation pushed the 17s Grade 2 slice past the 30s budget on the slower headless runner.
