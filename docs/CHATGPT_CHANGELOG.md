...
- Follow-up corrections (2026-07-11, same branch/PR, from reviewer feedback):
  1. **Concrete palette lock added.** The specs named palette families (`forest`/`arcane`/`wood_leather`/`metal`) but did not fix their hex values, which contract Section 4 requires before production art is committed (or independently generated assets drift). Added `docs/visual-targets/farm_palette.json` + `FARM_PALETTE.md`: a versioned, authoritative farm master palette (4 families, 42 swatches, shared anchors, flora accents, and the Wildbloom `pinnedAccents` mirrored from `WildbloomDiscoveryController.ts`). Extended `scripts/validate-visual-targets.mjs` to machine-check the lock (hex format, non-empty ramps, unique swatch names) so it cannot silently drift; every farm-scope targets doc now points at it.
  2. **13-tile blend sets completed.** Both `tile_farm_water_shore` (new) and the older `tile_farm_path_dirt` declared only 9 variants (center + 4 edges + 4 outer corners). Added the 4 missing inner corners (`inner_corner_ne/nw/se/sw`) to each, completing the contract Section 13 reduced 13-tile set — inner corners are what let paths and the pond follow concave, irregular outlines instead of only convex rectangles.
  3. **Fence/gate canvas heights corrected.** Both sit on the tall `actors_body` layer but were authored flat (`env_farm_fence` 16×16, `env_farm_gate` 32×16). Raised to `env_farm_fence` 16×32 (pivot `[8,31]`) and `env_farm_gate` 32×32 (pivot `[16,31]`), giving posts/rails visible height under the flattened "pop-up book" perspective (Section 4a) while keeping footprints (and therefore collision/gameplay) unchanged at 16×16 and 32×16.
  - Verification for the corrections: `npm run validate:visual-targets` (10 files, 36 targets, 1 palette; the palette validator confirmed via a negative test to reject bad hex/duplicate names), `npm run check`, `npm run test:asset-pipeline`, and `npm run test:unit` (48/48) all pass. The smoke suite is unaffected (no `src/`, map, or browser-runtime changes) and was 50/50 green earlier on this branch.
