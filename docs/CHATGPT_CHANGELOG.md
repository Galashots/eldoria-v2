...
- Follow-up corrections (2026-07-11, same branch/PR). An initial improvised pass (commit `99a3c6c`) added a palette lock with invented hex values; the reviewer then supplied an **approved style-lock reference** (external, uncommitted, STYLE REFERENCE ONLY) and precise corrections, so a second commit superseded the improvised palette with sampled-from-approved values and completed the geometry/handoff work:
  1. **Approved palette lock (supersedes the improvised one).** Removed `farm_palette.json`/`FARM_PALETTE.md` (invented colors) and added `docs/visual-targets/farm_environment_palette_v1.json` + `FARM_ENVIRONMENT_PALETTE_V1.md` with the reviewer's colors sampled from the approved farm-environment style-lock: families `forest` (6), `wood_leather` (5), `water` (3), `metal_stone` (5), plus the preserved authoritative Wildbloom identity accents (Root-Star/Moonwell/Foxfire) mirrored from `WildbloomDiscoveryController.ts`. Reworked `scripts/validate-visual-targets.mjs`'s palette check to the new schema (families as hex arrays, accents as hex arrays; confirmed via negative test to reject bad hex, empty arrays, and wrong status). All five farm-scope target docs repointed to the new file. The style-lock image itself is intentionally not committed.
  2. **13-tile blend sets completed.** Both `tile_farm_water_shore` and the older `tile_farm_path_dirt` declared only 9 variants; added the 4 inner corners (`inner_corner_ne/nw/se/sw`) to each for the full contract Section 13 reduced 13-tile set, so paths and the pond can follow concave, irregular outlines instead of only convex rectangles.
  3. **Fence/gate/signpost geometry corrected.** `env_farm_fence` → 16×32, footprint 16×16, pivot `[8,31]`, collision body `[0,16,16,16]`, with nine explicit orientation variants (`post`, `rail_horizontal`, `rail_vertical`, `corner_ne/nw/se/sw`, `broken_horizontal`, `broken_vertical`) so nothing depends on runtime rotation/mirroring (which would reverse the approved upper-left light). `env_farm_gate` → 32×32, footprint 32×16, pivot `[16,31]`, collision body and interaction both `[0,16,32,16]`. `env_farm_signpost` interaction corrected to the lower footprint `[0,16,16,16]`. Footprints/collision stay on the grounded lower band; only drawn height grows.
  4. **Production-generation handoff added.** `docs/art-pipeline/FARM_ENVIRONMENT_GENERATION_HANDOFF_V1.md` sequences ChatGPT source-art generation into Batches A–F (foundational candidates → terrain variations → vegetation → structures/props → water decoration → Wildbloom family), documenting per target/family the ID, variants, canvas/footprint/pivot, generation format, magenta-padding/seamless-terrain rules, source/manifest/output paths, normalization category, and audit criteria, plus the production rules (one-asset-per-image or same-size family sheet only, seamless centres, centre-before-edges, no baked shadows, mandatory source-audit verdict before normalization). The four production decisions it initially raised were subsequently resolved and recorded as final (see the next bullet); nothing remains open before Batch A.
  - Verification for the corrections: `npm run validate:visual-targets` (10 files, 36 targets, 1 palette), `npm run check`, `npm run test:asset-pipeline`, and `npm run test:unit` (48/48) all pass. The smoke suite is unaffected (no `src/`, map, or browser-runtime changes) and was 50/50 green earlier on this branch.
  - Handoff decisions resolved by the reviewer (folded into `FARM_ENVIRONMENT_GENERATION_HANDOFF_V1.md`): grass scatter → Batch C; tilled soil / crop sprouts / harvest / crop-row → Batch D; one deterministic packed runtime PNG per target ID with a fixed documented manifest layout (new §5.1 table; source may stay one image per variant); shimmer frames authored now with the runtime loop deferred to Phase 3; shore rocks stay 16×16 decals and the medium rock stays a 32×32 landmark.
  - Still true: no production farm-environment art exists yet; the next step is ChatGPT production generation using the handoff. PR #70 remains a target/specification PR, not the completed farm-kit PR, and stays draft.
