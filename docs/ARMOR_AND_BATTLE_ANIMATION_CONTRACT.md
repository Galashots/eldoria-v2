...
The editable source keeps these pieces separate and aligned to the base actor:
...
No independent scale, baseline, frame index, easing, or playback clock is permitted.
...
The outfit package synchronizes only to the Mage clips currently required by the base target: `idle`, `walk`, `cast`, and `hurt`.
...
The Ranger's current code-drawn ACTION recoil and tracking shot are bridge presentation. Dedicated Ranger sheets must replace that bridge before Ranger armor production begins.
...
- Camera shake, flashes, and particles require reduced-motion-safe behavior and later physical-iPad profiling.

## Production order

1. Continue the active Phase 2 farm-environment Batch B work.
2. Produce the dedicated Ranger Explorer base sheets.
3. Freeze Mage and Ranger clip names, frame counts, durations, pivots, and event frames.
4. Generate and audit the Mage starter source components against the frozen Mage base.
5. Compile and audit the Mage `outfit_back` and `outfit_front` runtime sheets plus the separate staff sheet.
6. Integrate a cosmetic whole-outfit toggle with no stats or save-schema expansion beyond what that focused feature requires.
7. Add merchant, inventory, equipment slots, prices, and gameplay bonuses only in a separately scoped progression milestone.
...
- no frame-to-frame drift at 1x and enlarged nearest-neighbour review scales;
...
- fresh in-engine screenshots for all directions and required clips;
