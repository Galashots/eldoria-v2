...
- `tile_farm_water_shore` **blend family (13 cells), v2 material correction** — deterministically composited by a `shoreline_bands` mode added to `scripts/compose-terrain-blend-family.mjs` (the merged `binary_material_interlock`/dirt mode is unchanged, regression-tested). Composites approved `water_a` (foreground/water) over approved `grass_a` (background/land) using the same four-corner mask reinterpreted as five q-bands (water / inner sand / outer sand / moss / grass), with a locked, edge-excluded hashed swatch variation (seed `0x0000570A`) from the locked `wood_leather`/`forest` palette only. `center` is byte-identical to approved `water_a`. ChatGPT's first (v1) visual review found the result read as a continuous bright outline with an inner-corner bullseye and near-invisible moss; ChatGPT prescribed an exact corrective material-selection recipe (same band geometry/topology/seed/palette), implemented verbatim as v2 — sand bands measurably darker/muted (14.0% mean luminance drop, 86.8% of sand pixels recoloured) with sand/moss/grass interlocking. The visual "continuous ring" character was muted, not eliminated, by design (v2 only reassigns swatches within existing bands); this is disclosed, not self-certified, pending ChatGPT's second verdict. Adjacency coverage is now exhaustive (100 directed pairs: 50 horizontal + 50 vertical across all 14 usable codes) and the band-class/sand-RGB mismatch gates are hard compositor failures, not just report fields; `npm run test:terrain-blend` now runs in CI. All gates pass (0 unexpected colours, exact band occupancy, 0 band-class/sand-RGB mismatches, packed `208×16` sheet with zero drift). Source + review evidence + packed sheet only. **Not yet approved** — ChatGPT's second formal visual verdict is pending; not runtime/map/Wangset integrated.
