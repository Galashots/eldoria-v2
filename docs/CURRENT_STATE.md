...
- **Dialogue read-aloud blips** (Model Council Batch 2): the typewriter voices each dialogue line with a soft per-character tick for the pre-reader, via a pure deterministic policy (`src/systems/textBlips.ts` — skips whitespace/punctuation, blips every 2nd voiced glyph) that suppresses while a line is TTS-voiced (one voice per line: Ranger/manual get blips, Mage `autoRead` lets TTS be the voice). Routed through `playSfx` and the global mute. The blip key uses a **dedicated `TEXT_BLIP_COOLDOWN_MS` (40 ms)** rather than the default 90 ms tap-spam guard, because at 24 ms/char a throttle-2 blip is requested every ~48 ms — the 90 ms guard would drop every other one and the *audible* cadence would collapse to ~every 4th glyph. The 40 ms cooldown clears the 48 ms spacing, so the documented "every 2nd voiced glyph" is what a player actually hears (invariant `TEXT_BLIP_COOLDOWN_MS < throttle × TYPEWRITER_MS_PER_CHAR` is asserted in `tests/unit/textBlips.test.ts`). The placeholder `text-blip.wav` is synthesized by `scripts/generate-blip-sfx.mjs`.
...
Both required reduced terrain-blend families are approved and on `main`: `tile_farm_path_dirt` and `tile_farm_water_shore`. The farm proof map now uses both families' transition cells (dirt-blend and shoreline), assigned by the pure neighborhood resolver so the path and pond blend into the grass; only the final Wangset-aware/Tiled-native authoring pass remains deferred.
...
4. The dirt and shoreline transition families are now integrated into the farm proof map (PR #114) via the neighborhood resolver; the remaining terrain work is the final Wangset-aware/Tiled-native composition that demonstrates coherent boundaries at target scale, plus extending blended terrain beyond the farm.
...
- The farm terrain proof now blends pond/path edges into the grass via the integrated dirt-blend and shoreline transition cells (PR #114), but it is still a resolver-driven proof, not the final Wangset-aware/Tiled-native composition; boundary coherence at target scale must still be validated in that final pass, and woods/village keep hard edges.
