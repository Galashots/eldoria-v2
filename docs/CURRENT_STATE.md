...
- **Dialogue read-aloud blips** (Model Council Batch 2): the typewriter voices each dialogue line with a soft per-character tick for the pre-reader, via a pure deterministic policy (`src/systems/textBlips.ts` — skips whitespace/punctuation, every 2nd voiced glyph) that suppresses while a line is TTS-voiced (one voice per line: Ranger/manual get blips, Mage `autoRead` lets TTS be the voice). Routed through `playSfx` for the per-key anti-buzz cooldown and global mute; the placeholder `text-blip.wav` is synthesized by `scripts/generate-blip-sfx.mjs`.
